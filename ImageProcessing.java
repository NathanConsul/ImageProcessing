...
public class ImageProcessing {
    ...
    public static void main(String[] args) {
		...
		int[][] imageData = imgToTwoD("inputFileOrLink.jpg");		
		
		// viewImageData(imageData);
		
		int[][] trimmed = trimBorders(imageData, 60);
		twoDToImage(trimmed, "filename.jpg");

        int[][] negative = negativeColor(imageData);
        twoDToImage(negative, "filename.jpg");

        int[][] stretchedHImg = stretchHorizontally(imageData);
        twoDToImage(stretchedHImg, "filename.jpg");

        int[][] shrinkVImg = shrinkVertically(imageData);
        twoDToImage(shrinkVImg, "filename.jpg");

        int[][] invertedImg = invertImage(imageData);
        twoDToImage(invertedImg, "filename.jpg");

        int[][] coloredImg = colorFilter(imageData, -75, 30, -30);
        twoDToImage(coloredImg, "filename.jpg");		
        
		// Painting with pixels
        int[][] blankImg = new int[500][500];

        int[][] randomImg = paintRandomImage(blankImg);
        twoDToImage(randomImg, "filename.jpg");

        int[] rgba = {255, 255, 0, 255};

        int[][] rectangleImg = paintRectangle(randomImg, 200, 200, 100, 100, getColorIntValFromRGBA(rgba));
        twoDToImage(rectangleImg, "filename.jpg");

        int[][] generatedRectangles = generateRectangles(randomImg, 1000);
        twoDToImage(generatedRectangles, "filename.jpg");
	}
        ...
        int[][] manipulatedImg = new int[imageTwoD.length][imageTwoD[0].length];
            for (int i = 0; i < imageTwoD.length; i++)  {
                for (int j = 0; j < imageTwoD[i].length; j++) {
                    manipulatedImg[i][j] = imageTwoD[(imageTwoD.length - 1) - i][(imageTwoD[i].length - 1) - j];
                }
            }
		return manipulatedImg;
	}
